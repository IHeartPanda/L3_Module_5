...
public class LongChipCompetition {
	...
	public static void main(String[] args) {
		LongChipCompetition lc = new LongChipCompetition();
		lc.initializeBeatles();
		String beat = "";
		double chippy = 0;
		for (int i = 0; i < theBeatles.size(); i++) {
			for (int j = 0; j < theBeatles.get(i).getChips().size(); j++) {
				if (chippy < theBeatles.get(i).getChips().get(j).getLength()) {
					chippy = theBeatles.get(i).getChips().get(j).getLength();
					beat = theBeatles.get(i).getName();
				}
			}
		}
		System.out.println(beat + " has the longest chip.");
	}
